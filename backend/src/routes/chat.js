...
import { CHAT } from '../constants/apiEndpoints.js';
...
// Block policy (hide-but-retain): conversations with a blocked pair are excluded
// from BOTH the blocker's list AND the blockee's list — but Message documents are
// never deleted, preserving evidence for moderation.
router.get(CHAT.CONVERSATIONS, userAuth, async (req, res) => {
  try {
    const loggedInUserId = req.user._id;

    // Build a set of blocked user IDs in both directions:
    // 1. Users I have blocked (already on req.user, no extra query)
    const myBlockedIds = new Set(req.user.blockedUsers.map((id) => id.toString()));

    // 2. Users who have blocked me (one index-friendly query)
    ...
    const data = conversations
      ...
      // Hide conversations where a block exists in either direction
...
router.post(CHAT.GET_OR_CREATE, userAuth, async (req, res) => {
  try {
    const loggedInUserId = req.user._id;
...
// GET /chat/conversations/:conversationId/messages — paginated history (newest-first pages, oldest-first within page)
router.get(CHAT.MESSAGES, userAuth, async (req, res) => {
  try {
    const loggedInUserId = req.user._id;
    ...
    res.status(200).json({
      ...
      page,
      data: messages.reverse(), // oldest-first for easy rendering
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// POST /chat/conversations/:conversationId/read — mark conversation as read (per-conversation last-read timestamp)
router.post(CHAT.READ, userAuth, async (req, res) => {
  try {
    const loggedInUserId = req.user._id;
    const { conversationId } = req.params;
